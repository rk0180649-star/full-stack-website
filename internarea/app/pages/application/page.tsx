...
const page = () => {
  ...
  const [data, setdata] = useState <any>([]);
       useEffect(()=>{
          const fetchdata = async () =>{
            try{
              const res=await axios.get("https://full-stack-website-h8ju.onrender.com/api/application")
              setdata(res.data);
  ...
  try {
    const res = await axios.put(`https://full-stack-website-h8ju.onrender.com/api/application/${id}`, { action });
                ...
                <tr key={application._id} className="hover:bg-gray-50/50 transition-colors">
                  ...
                  <td className="py-4 px-6">
                    <div className="flex items-center gap-2">
                      <Link
                        href={`/pages/detailapplication/${application._id}`}
                        className="text-blue-600 font-medium hover:underline text-xs mr-1"
                      >
                        View Details
                      </Link>
                      <button
