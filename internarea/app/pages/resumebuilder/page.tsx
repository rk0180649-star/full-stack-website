...
// TypeScript global declaration for Razorpay window object
declare global {
  interface Window {
    Razorpay: any;
  }
...
export default function ResumeBuilder() {
  ...
  // Agar user logged in hai, to uska email aur name set karein
  useEffect(() => {
    if (user?.email) {
      setFormData((prev) => ({
        ...prev,
  ...
  // Baaki code (themeColor, state, functions wagairah) yahan se shuru hoga...

  const [themeColor, setThemeColor] = useState("#2563eb");
  const [selectedTemplate, setSelectedTemplate] = useState("modern");

  // Flow State: 'idle' | 'otp' | 'ready_to_pay' | 'download_ready'
  ...
  const handleInitiateDownload = async () => {
    ...
    try {
      ...
      alert("Backend server se connect nahi ho paya. Make sure backend is running.");
    } finally {
      setLoading(false);
    }
  };
  ...
  // Step 3: Razorpay Payment Initiation (₹50)
...
console.log(orderData)
      const options = {
        key: process.env.NEXT_PUBLIC_RAZORPAY_KEY_ID || "rzp_test_TXTSZuElZoacKt",
        amount: orderData.amount,
        currency: "INR",
        name: "Resume Builder Premium",
        description: "ATS Resume PDF Download & Profile Link",
        order_id: orderData.id,
        handler: async function (response: any) {
          // Step 4: Verify Payment & PDF Generation
          try {
      ...
      <div className="min-h-screen bg-slate-100 flex flex-col">
        ...
        {/* Modal: Verification & Success */}
            ...
            <div 
              ...
              onClick={() => setIsModalOpen(false)} // Bahar click karne par modal band hoga
            >
              <div 
